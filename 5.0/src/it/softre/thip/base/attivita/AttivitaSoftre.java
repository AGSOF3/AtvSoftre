...
public class AttivitaSoftre extends AttivitaSoftrePO {
	...
	protected void copiaValoriInOldTestata() {
		...
		iOldAttivita.setIdIncaricato(getIncaricato() != null ? getIncaricato().getIdDipendente() : null);
	...
	public int save() throws SQLException {
		...
		calcolaDataInizioTeorica();

		//..Se completata allora valorizzo la data di completamento
		if(getStatoAttivita() == COMPLETATA) {
			setDataCompletamento(TimeUtils.getCurrentDate());
		}else if(getOldAttivita() != null && getOldAttivita().getStatoAttivita() == COMPLETATA) { //.Se la riapro o cambio stato la svuoto
	...
	public boolean isAssegnazioneDaNotificare(boolean isOnDB) {
		...
		if(!isOnDB) { //se e' new ma ha l'incaricato
			return true;
		}else if(isOnDB && getOldAttivita() == null) {
			return false;
		}else if((getOldAttivita().getIncaricato() == null || !(getIdIncaricato().equals(getOldAttivita().getIdIncaricato())))) {
			return true;
		}
		return false;
	}
	...
	protected void notificaIncaricatoAssegnazioneAttivita() {
		...
		try {
			...
			if(utente.getDipendente() != null && assegnatario != null) {
				...
				ClassAD statoAttivita = hdr.getAttribute("StatoAttivita");
				...
				String htmlContent = "<!DOCTYPE html>" +
						...
						"<p><strong>Stato attivita:</strong> "+(((EnumType)statoAttivita.getType()).descriptionFromValue(String.valueOf(getStatoAttivita())))+"</p>";
				...
				if(getRichiedenteSoftre() != null) {
					...
					Dipendente richiedenteSoftre = getRubricaSoftre();
					if(richiedenteSoftre != null) {
						richiedente = richiedenteSoftre.getCognome() + " " + richiedenteSoftre.getNome();
						htmlContent += 
								"<p><strong>Richiedente Softre:</strong> "+richiedente+" </p>";
					}
				}
				if(getRichiedenteCliente() != null) {
					RubricaEstesa rubrica = getRubricaCliente();
					if(rubrica != null) {
						richiedente = rubrica.getCognome() + " " + rubrica.getNome();
						htmlContent += 
	...
	protected String getCollaboratoriConcatenati(Dipendente incaricato,String concatenazione) {
		if(!isOnDB()) {
			...
			while(collaboratori.hasNext()) {
				...
				result = collaboratore.getUtente().getNome() + " " + collaboratore.getUtente().getCognome();
	...
	protected void controllaIncaricatoSeCollaboratore() throws SQLException {
		if(getIncaricato() != null) {
		...
		}else if(getOldAttivita() != null){
			List<AttivitaCollaboratore> collaboratori = getAttivitaCollaboratori();
			String idOldIncaricato = getOldAttivita().getIdIncaricato();
			Iterator iterCollaboratori = collaboratori.iterator();
			while(iterCollaboratori.hasNext()) {
				...
				if(collaboratore.getUtente().getIdDipendente().equals(idOldIncaricato)) {
					iterCollaboratori.remove();
				}
			}
		}
	...
	protected void calcolaDataInizioTeorica() {
		...
		if((getQuotazioneGg() == null || getQuotazioneGg().compareTo(BigDecimal.ZERO) == 0)
				&& getDataPrevistaConsegna() != null) {
			setDataInizio(getDataPrevistaConsegna());
		}
	}
	...
	public void retrieveRelatedEmployees(){
		...
		while (iterCollaboratori.hasNext()) {
			...
			if (!collaboratore.getIdUtente().equals(UtenteAzienda.getUtenteAziendaConnesso().getIdUtente())) {
				...
				Dipendente dipendente = collaboratore.getUtente();
